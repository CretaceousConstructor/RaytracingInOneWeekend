...
scene::scene()
{
	...
	const std::shared_ptr<tracer> tracer_p = std::make_shared<pathTracer>(&world);
	tracer_p->set_back_ground_color(backgroundcolor);

	world.set_tracer_ptr(tracer_p);
	cam->set_tracer_ptr(tracer_p);
}

void scene::render(std::ofstream &result)
{
	cam->render_scence(world, lights, result, renstate);
}
...
void scene::random_balls_scene()
{
	...
	world.add_object(make_shared<sphere>(point3(0., -1000., 0.), 1000., make_shared<diffuse>(checker)));
	...
	world.generating_acceleration_structure(start_time, end_time);
	//================================================================================================
}
...
void scene::two_balls_scene()
{
	...
	world.add_object(make_shared<sphere>(point3(0, -10, 0), 10, make_shared<diffuse>(checker)));
	world.add_object(make_shared<sphere>(point3(0, 10, 0), 10, make_shared<diffuse>(checker)));
...
void scene::two_perlin_spheres()
{
	...
	world.add_object(make_shared<sphere>(point3(0, -1000, 0), 1000, make_shared<diffuse>(pertext)));
	world.add_object(make_shared<sphere>(point3(0, 2, 0), 2, make_shared<diffuse>(pertext)));
...
void scene::sphere_texture_scene()
{
	...
	world.add_object(instance_globe);
	world.generating_acceleration_structure(start_time, end_time);
}
...
void scene::rectangle_scene()
{
	...
	world.add_object(make_shared<rectangle>(vec3(2., 0., 0.), vec3(0., 2., 0.), point3(3.0, 1.0, -2.0), vec3(0., 0., 1.), difflight));
}
...
void scene::cornell_box()
{
	...
	world.add_object(green_wall);
	world.add_object(red_wall);
	world.add_object(light_obj);

	world.add_object(white_wall0);
	world.add_object(white_wall1);
	world.add_object(white_wall2);

	world.add_object(white_box0);
	world.add_object(white_box1);
...
void scene::testing_scene()
{
	...
	//world.add_object(make_shared<rectangle>(vec3(0., 0., -555.0), vec3(0., -555., 0.), point3(555., 555., 555.0), vec3(-1., 0., 0.), green));
	//world.add_object(make_shared<rectangle>(vec3(0., 0., -555.0), vec3(0., -555., 0.), point3(0., 555., 555.), vec3(1., 0., 0.), red));

	//world.add_object(make_shared<rectangle>(vec3(130., 0., 0.0), vec3(0., 0., -105.), point3(213., 554., 332.), vec3(0., -1., 0.), light));

	//world.add_object(make_shared<rectangle>(vec3(555., 0., 0.0), vec3(0., 0., -555.), point3(0., 0., 555.), vec3(0., 1., 0.), white));

	//world.add_object(make_shared<rectangle>(vec3(555., 0., 0.0), vec3(0., 0., -555.), point3(0., 555., 555.), vec3(0., -1., 0.), white));

	//world.add_object(make_shared<rectangle>(vec3(555., 0., 0.0), vec3(0., 555., 0.), point3(0., 0., 555.), vec3(0., 0., -1.), white));

	//world.add_object(make_shared<box>(point3(-2., 0., -4.), point3(2., 4., 0), white));
...
void scene::cornell_smoke()
{
	...
	const auto green_wall = make_shared<rectangle>(vec3(0., 0., -555.0), vec3(0., -555., 0.), point3(555., 555., 555.0), vec3(-1., 0., 0.), green);
	const auto red_wall   = make_shared<rectangle>(vec3(0., 0., -555.0), vec3(0., -555., 0.), point3(0., 555., 555.), vec3(1., 0., 0.), red);
	const auto light_obj  = make_shared<rectangle>(vec3(300., 0., 0.0), vec3(0., 0., -305.), point3(133., 554., 432.), vec3(0., -1., 0.), light);
	...
	white_box0->SetRotation(0., glm::radians(15.0), 0.);
	white_box0->SetTranslation(vec3(265, 1, 295));
	white_box1->SetRotation(0., glm::radians(-18.0), 0.);
	white_box1->SetTranslation(vec3(130, 1, 65));
	...
	world.add_object(make_shared<constant_medium>(white_box0, 0.01, color(0, 0, 0)));
	world.add_object(make_shared<constant_medium>(white_box1, 0.01, color(1, 1, 1)));
}
...
void scene::final_scene()
{
	...
	for (int i = 0; i < boxes_per_side; i++)
	{
		for (int j = 0; j < boxes_per_side; j++)
		{
			...
			world.add_object(make_shared<box>(point3(x0, y0, z0), point3(x1, y1, z1), ground));
		}
	}
