...
sampler::sampler(unsigned int numSamples, unsigned int numSets) :
    num_samples(numSamples),
    num_sets(numSets),
    sqrt_of_numSamples(static_cast<unsigned int>(glm::sqrt(numSamples))),
    setNumberDistribution(0, numSets - 1),        //[0,numSets)
    samplePerPiexlDistribution(0, static_cast<unsigned int>(glm::sqrt(numSamples)) * static_cast<unsigned int>(glm::sqrt(numSamples)) - 1)
{
	if (numSamples < 1 || numSets < 1)
	{
		throw std::runtime_error("Too small a number");
	}
...
void sampler::map_square_samples_to_hemisphere(const double e)
{
	...
	const auto     size = unit_square_samples.size();
	hemisphere_samples.reserve(num_samples * num_sets);

	for (size_t j = 0; j < size; j++)
	{
		const double cos_phi   = glm::cos(2.0 * PI * unit_square_samples[j].x);
		const double sin_phi   = glm::sin(2.0 * PI * unit_square_samples[j].x);
		const double cos_theta = glm::pow((1.0 - unit_square_samples[j].y), 1. / (e + 1.));
		const double sin_theta = glm::sqrt(1.0 - cos_theta * cos_theta);
		const double pu        = sin_theta * cos_phi;
		const double pv        = sin_theta * sin_phi;
		const double pw        = cos_theta;

		hemisphere_samples.emplace_back(TrekMath::point3(pu, pv, pw));        //这里仅仅给定u ,v ,w轴的系数
	}
}
